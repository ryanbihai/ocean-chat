...
class GPTImageEditTool(Tool):
    ...
    def _invoke(
        ...
        """
        endpoint = self.runtime.credentials.get("skyrouter_endpoint", None)
        
        if not endpoint:
            yield self.create_text_message("Error: Skyrouter endpoint is required.")
            return

        client = OpenAI(
            api_key=self.runtime.credentials["skyrouter_api_key"],
            base_url=endpoint,
        )
