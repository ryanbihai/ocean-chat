...
```json5
{
  ...
  "contacts": [
    {
      ...
      // ── 多身份：一个联系人可以有多个 Agent ──
      "agents": [
        {
          "agentId": "01JQRS9XYZ...",              // OceanBus UUID
          "openId": "oa9EliN5y6H...",              // 稳定 OpenID（用于发消息）
          "purpose": "日常助手",                     // 这个 Agent 的用途
          "isDefault": true                         // 默认发到哪个 Agent
        },
        {
          "agentId": "02ABCD...",
          "openId": "obXyz...",
      ...
      // ── 标签（LLM 自动维护，只改 tags，不碰名字和 Agent 信息）──
      ...
      // ── 时间戳 ──
      "lastContactAt": "2026-05-06T12:00:00Z",     // 最后一次互动（发消息/收消息/约见）
      "createdAt": "2026-04-01T10:00:00Z",
      "updatedAt": "2026-05-06T15:30:00Z",

      // ── 来源（保留向后兼容，推荐使用 provenance）──
      "source": "manual",

      // ── 来源追踪 v2（三元组：哪个系统 + 源ID + 时效性）──
      "provenance": {
        "account": "yellow-pages",       // 来源系统
        "sourceId": "yp_entry_123",      // 源系统中的 ID（用于关联和校验）
        "firstSeenAt": "2026-04-01T10:00:00Z",
        "lastVerifiedAt": "2026-05-06T15:30:00Z"  // 源数据最后确认有效的时间
      },

      // ── 状态 ──
      // "active"   — 正常
      // "pending"  — 待审核（autoDiscovery 产出）
      // "archived" — 已归档（软删除）
      "status": "active",

      // ═══════════════════════════════════════════════════
      // 层2: 应用扩展 — 各消费者读写自己的命名空间
      // ═══════════════════════════════════════════════════
      "apps": {
        "ocean-agent": {
          "stage": "方案已发",
  ...
  "identities": [
    ...
    },
    {
      ...
      "purpose": "保险业务专用",
      "agents": [
        { "agentId": "ddd37d...", "openId": "ocWork...", "isDefault": true }
      ]
    }
  ],
  ...
  "autoDiscovery": {
    "enabled": true,
    "minMentions": 3,                             // 新名字出现 N 次后加入 pending
    "sources": ["chat.log", "user-messages"],      // 扫描来源
    "ignoreList": ["我", "你", "他", "大家"],        // 忽略词
    "pending": [
      {
        "id": "auto_lili",
        "name": "李丽",
  ...
  // ═══════════════════════════════════════════════════
  // 自动重复检测 v2（add() 时自动检测，LLM 可据此建议合并）
  // ═══════════════════════════════════════════════════
  "duplicateHints": [
    {
      "contactA": "laowang",
      "contactB": "wangzong",
      "reason": "same_openid",        // "same_openid" | "same_agentId" | "name_similarity"
...
interface MatchEntry {
  id: string;
  name: string;
  matchField: "name" | "alias" | "id" | "tag" | "note";
  highlight: string;      // 匹配的字段值
  tags: string[];
  notes: string;          // 前 80 字
  agents: AgentRef[];     // 含 purpose，用于 LLM 区分重名人
  source: string;
}
```
...
#### `roster.findByAgentId(agentId: string): Contact | null`

收到消息时，通过 `from_agent_id` 反查联系人（走 byAgentId 索引）。O(1)。
...
interface RosterFilter {
  ...
  source?: string;        // "manual" | "yellow-pages" | "auto-discovery" | "chat"
...
#### `roster.update(id: string, patch: Partial<Contact>): Contact`
...
合并重复联系人。discardId 的 agents、aliases、tags 合并到 keepId，discardId 软删除。

#### `roster.addAlias(id: string, alias: string): void`
...
→ 回复：
  通讯录里有两个"老王"：
  1. 老王 — 大学同学，标签: friend, badminton（日常助手）
  2. 老王 — 公司财务，标签: colleague, finance（工作助手）
  你找哪个？
```
...
LLM 可以在以下时机更新 tags（**只改 tags，不碰名字和 Agent 信息**）：
...
if (results.exact.length === 1) {
  ...
  const defaultAgent = contact.agents.find(a => a.isDefault);
  await ob.send(defaultAgent.openId, "周五打球？");
  ob.roster.touch(contact.id);  // 更新 lastContactAt
}

// 收到消息时：反向查人
ob.startListening(async (msg) => {
  const contact = ob.roster.findByAgentId(msg.from_agent_id);
