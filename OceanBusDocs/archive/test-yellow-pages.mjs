 ...
 *   4. startHeartbeat    — 自动心跳（启动/停止/状态查询 + 自定义间隔）
 ...
 *
 * v2 新增演示:
 *   - setIdentity() + 短签名方法（无需每次都传 openid/signer）
 *   - createServiceKey() 一行生成密钥+签名器
 *   - startHeartbeat({ intervalMs }) 无需创建新 SDK 实例
 *   - destroy() 自动清理黄页条目
 */
...
const { generateKeypair, sign, keypairToBase64url } = require('./dist/crypto/ed25519.js');
...
async function main() {
  ...
  // ── v2: createServiceKey() 一行搞定密钥+签名器 ──
  const svc = await ob.createServiceKey();
  console.log(`  公钥:     ${svc.publicKey.substring(0, 40)}...`);

  // 设置默认身份——后续调用无需再传 openid 和 signer
  ob.l1.yellowPages.setIdentity(myOpenid, svc.signer, svc.publicKey);

  // 准备一个错误签名器用于验证签名拒绝
  const wrongKeypair = await generateKeypair();
  const wrongSigner = async (p) => sign(wrongKeypair.secretKey, p);
  ...
  // 1.1 正常注册（短形式：只传 tags + description）
  console.log('\n  1.1 正常注册（短形式——无需传 openid/signer）');
  ...
  await check('拒绝重复 openid', async () => {
    const r = await ob.l1.yellowPages.registerService(['other'], '重复注册应被拒绝');
    if (r.code !== 1002) throw new Error(`expected 1002, got ${r.code}`);
  });

  // 1.3 错误签名——长形式显式传参测试签名验证
  ...
  await check('错误签名被拒绝', async () => {
    const r = await ob.l1.yellowPages.registerService(
      openid2, ['test'], '错误签名测试', svc.publicKey,
      async (p) => sign(wrongKeypair.secretKey, p)
    );
    if (r.code !== 1001) throw new Error(`expected 1001, got ${r.code}`);
  });

  // 验证长形式（显式传参）心跳在 3.4 中测试
  await ob2.destroy();
  ...
  // 2.3 分页
  console.log('\n  2.3 分页翻页');
  const all = await ob.l1.yellowPages.discover(undefined, 2);
  ...
  // 3.1 正常心跳（短形式：无参数）
  await check('正常心跳返回 code=0（短形式——无参数）', async () => {
    const r = await ob.l1.yellowPages.heartbeat();
    if (r.code !== 0) throw new Error(`code=${r.code}`);
  });
  ...
  await check('不存在条目心跳被拒绝', async () => {
    const fakeSigner = async (p) => `ed25519:${'A'.repeat(86)}`;
  ...
  // 3.4 长形式心跳仍可用
  console.log('\n  3.4 长形式显式传参（向后兼容）');
  await check('长形式心跳正常', async () => {
    const r = await ob.l1.yellowPages.heartbeat(myOpenid, svc.signer);
    if (r.code !== 0) throw new Error(`code=${r.code}`);
  });

  // ═══════════════════════════════════════════════════════
  // 4. startHeartbeat — v2: 支持 intervalMs 覆盖
  // ═══════════════════════════════════════════════════════
  hr('4. startHeartbeat / stopHeartbeat — 自动心跳（v2: 无需新实例）');

  // 4.1 短形式：无参数启动（使用默认 5 分钟间隔）
  await check('初始状态 isHeartbeating() = false', () => {
    if (ob.l1.yellowPages.isHeartbeating()) throw new Error();
  });

  // 短形式启动（使用 setIdentity 的身份 + 自定义间隔 10 秒）
  console.log('  短形式启动自动心跳（intervalMs=10000，无需创建新 SDK 实例）...');
  ob.l1.yellowPages.startHeartbeat({ intervalMs: 10000 });

  await check('startHeartbeat 后 isHeartbeating() = true', () => {
    if (!ob.l1.yellowPages.isHeartbeating()) throw new Error();
  });

  console.log('  等待 5 秒验证启动成功...');
  await sleep(5000);

  ob.l1.yellowPages.stopHeartbeat();
  ...
  ob.l1.yellowPages.startHeartbeat({ intervalMs: 3000, onError: (e) => { heartbeatErrorCount++; } });
  await sleep(2000);
  ob.l1.yellowPages.stopHeartbeat();

  // 4.3 恢复长间隔自动心跳（后续测试需要）
  console.log('\n  4.3 恢复默认间隔自动心跳');
  ob.l1.yellowPages.startHeartbeat({ intervalMs: 60000 }); // 1 分钟，给后续测试留时间
  ...
  // 5.1 短形式：只传 tags 和 description
  console.log('\n  5.1 短形式 updateService(tags, description)');
  let updatedEntry;
  ...
  // 5.2 短形式：只更新 tags
  console.log('\n  5.2 短形式 updateService(tags)——只更新 tags');
  await ob.l1.yellowPages.updateService(['food', 'dumpling', 'delivery']);
  ...
  // ═══════════════════════════════════════════════════════
  // 6. deregisterService + 重新注册
  // ═══════════════════════════════════════════════════════
  hr('6. deregisterService — 注销服务');

  await check('正常注销返回 code=0（短形式——无参数）', async () => {
    const r = await ob.l1.yellowPages.deregisterService();
    if (r.code !== 0) throw new Error(`code=${r.code}`);
  });
  ...
  await check('本地拦截超长 tags', async () => {
    try {
      await ob.l1.yellowPages.registerService(longTags, 'valid description');
  ...
  await check('本地拦截超长 description', async () => {
    try {
      await ob.l1.yellowPages.registerService(['ok'], 'x'.repeat(801));
      throw new Error('未抛出异常');
    } catch (e) {
  ...
  // 9.1 setIdentity + 全短形式调用链
  console.log('\n  9.1 setIdentity + 全短形式调用链');
  const ob3 = await createOceanBus();
  await ob3.register();
  const openid3 = await ob3.getOpenId();
  const svc3 = await ob3.createServiceKey();
  ob3.l1.yellowPages.setIdentity(openid3, svc3.signer, svc3.publicKey);

  await check('短形式 registerService', async () => {
    const r = await ob3.l1.yellowPages.registerService(['v2-test'], 'v2 短形式全链路测试');
    if (r.code !== 0) throw new Error(`code=${r.code}`);
  });
  await check('短形式 heartbeat', async () => {
    const r = await ob3.l1.yellowPages.heartbeat();
    if (r.code !== 0) throw new Error(`code=${r.code}`);
  });
  await check('短形式 startHeartbeat', () => {
    ob3.l1.yellowPages.startHeartbeat({ intervalMs: 10000 });
    if (!ob3.l1.yellowPages.isHeartbeating()) throw new Error();
  });
  await check('短形式 updateService', async () => {
    const r = await ob3.l1.yellowPages.updateService(['v2-updated'], 'v2 更新测试');
    if (r.code !== 0) throw new Error(`code=${r.code}`);
  });

  // 9.2 destroy() 自动清理黄页
  console.log('\n  9.2 destroy() 自动清理黄页');
  console.log('      setIdentity 已设置 → destroy 将自动 deregister...');
  await ob3.destroy();
  console.log('      destroy 完成（自动 deregister 静默执行）');

  // 验证自动清理后，条目确实被注销
  const postDestroy = await ob.l1.yellowPages.discover(['v2-test'], 5);
  await check('destroy 后条目已自动注销', () => {
    const found = postDestroy.data.entries.find(e => e.openid === openid3);
    if (found) throw new Error('条目仍存在');
  });
  ...
  // stop any remaining heartbeat
  ob.l1.yellowPages.stopHeartbeat();
  // destroy 会自动 deregister（因为 setIdentity 已设置）
  await ob.destroy();
  console.log('  已清理');
