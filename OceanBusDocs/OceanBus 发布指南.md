...
### 1.4 发布到 GitHub（第一步，必须先做）

> **铁律**：先 GitHub，后 ClawHub。GitHub 是代码源，ClawHub 是分发渠道。永远不要跳过 GitHub 直接发 ClawHub。

#### 新 Skill 首次发布

```bash
# 在 monorepo 根目录，skill 代码已在 skills/<slug>/ 下开发完成

# 1. 在 GitHub 上创建独立仓库（手动操作）
#    https://github.com/new → 仓库名与 skill slug 一致 → 公开 → 不要勾选 README

# 2. 将 skill 子目录推送到独立仓库
git remote add skill-<slug> https://github.com/ryanbihai/<slug>.git
...
#### 已有 Skill 日常更新（推荐：使用脚本）

```bash
# 一条命令搞定 GitHub push + tag + ClawHub 发布
pwsh scripts/publish-skill.ps1 ocean-chat 2.9.3 -Changelog "修复 Roster 重复检测"

# 脚本内部自动执行：
#   [1/3] git subtree push → 推送到独立 GitHub 仓库
#   [2/3] git tag + push    → 版本标记
...
cd /tmp/ocean-chat
git tag v2.9.3 -m "ocean-chat v2.9.3 — 修复 Roster 重复检测"
git push origin v2.9.3
rm -rf /tmp/ocean-chat
```

> **`git subtree push` 会改写目标仓库的历史。** 这是安全的——独立仓库只是发布镜像，没有人直接在上面开发。如果 push 失败，使用方式 B 的 force push 即可。
...
### 1.5 发布到 ClawHub（第二步，GitHub 成功后才能做）

> **ClawHub 从 GitHub 读取代码。** GitHub 上的 release 就绪后，才能发 ClawHub。
...
```bash
# 推荐：使用脚本自动处理（从 GitHub clone → clawhub publish）
pwsh scripts/publish-skill.ps1 ocean-chat 2.9.3

# 或手动：从独立 GitHub 仓库 clone 后发布
git clone https://github.com/ryanbihai/ocean-chat.git /tmp/ocean-chat
clawhub publish /tmp/ocean-chat \
  --slug "ocean-chat" \
  --name "Ocean Chat" \
  --version "2.9.3" \
  --changelog "修复 Roster 重复检测逻辑"
rm -rf /tmp/ocean-chat
```
...
```
完整流程（GitHub 先，ClawHub 后）：

  1. 在 monorepo 修改代码
  2. 更新 SKILL.md 和 manifest.yaml 中的 version
  3. 提交到 monorepo: git commit -m "feat: xxx"
  4. git subtree push → 推送到独立 GitHub 仓库
  5. 在独立仓库打 tag: git tag vX.Y.Z && git push origin vX.Y.Z
  6. clawhub publish → ClawHub 上线
  7. 验证: clawhub inspect <slug>
```

**为什么必须是这个顺序**：
- ClawHub 从 GitHub 读取 SKILL.md，如果 GitHub 上 version 还是旧的，ClawHub 发布会被拒
- GitHub tag 是版本的权威记录——即使 ClawHub 发布失败，代码版本历史是完整的
- 出现问题回滚时，GitHub 历史可追溯
...
具体的发布命令（git subtree push、git tag、clawhub publish）和脚本用法，详见 [1.4 发布到 GitHub](#14-发布到-github第一步必须先做) 和 [1.5 发布到 ClawHub](#15-发布到-clawhub第二步github-成功后才能做)。此处仅列出 monorepo 特有的维护操作。

#### 查看各 Skill 的 SDK 依赖版本

```bash
node scripts/bump-sdk.js --status
```

#### SDK 发新版后统一升级所有 Skill

```bash
# 步骤 1: 升级 SDK 自身版本号 + npm publish（详见第二章）
# 步骤 2: 在 monorepo 中统一升级所有 Skill 的依赖声明
...
# 步骤 5: 逐个发布受影响的 Skill（详见 1.4 + 1.5）
