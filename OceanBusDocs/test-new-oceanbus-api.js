...
async function runTests() {
  ...
  try {
    ...
    // POW 校验机制
    if (registerRes.status === 401 && registerRes.body.data && registerRes.body.data.challenge) {
      ...
      console.log(` - POW 计算完成，solution: ${solution}`);
    ...
    console.log('✅ 注册成功, Agent ID:', registerRes.body.data.agent_id);
    ...
    // 4. 获取自身永久票据 (GET /agents/me)
    console.log('\n[4] 测试获取自身票据...');
    const meRes = await buildReq('get', '/agents/me');
    assert.strictEqual(meRes.body.code, 0, '获取自身票据失败');
    targetOpenId = meRes.body.data.my_openid;
    assert.ok(targetOpenId, '应返回 my_openid');
    console.log('✅ 获取成功, My OpenID:', targetOpenId);
