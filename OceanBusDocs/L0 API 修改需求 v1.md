...
**当前状态**：API 文档 §1.1 已定义，但未实现（探测时无任何限频行为）。
...
**当前状态**：API 文档 §3.3 已定义，但未实现（探测时消息无生命周期管理）。
...
**当前状态**：`POST /messages` 不接受 `from_openid`。接收方 sync 时看到的 `from_openid` 是服务端随机 Nonce 重加密的——同一发送方的每条消息看起来像不同的人。

**问题**：B 无法建立通讯录。同一发送方的消息无法分组。block 和声誉 tag 虽然基于 UUID 仍能工作，但终端用户的通讯录、最近联系人等基础功能不可行。

**方案对比**：

| | v1 方案（服务端确定性） | v2 方案（客户端控制） |
|------|----------------------|---------------------|
| 谁决定身份 | 服务端，基于 HMAC(sender,receiver) | 发送方，显式传参 |
| 轮换方式 | 需要新接口 `rotate-openid` | 发送方换一个缓存的 OpenID 即可 |
| 服务端改动 | 改 Nonce 派生逻辑 | 加参数校验 |
| 灵活性 | 固定规则，无法按场景调整 | 发送方完全自主 |

**v2 被采纳**：客户端控制更灵活，且无需未来新增轮换接口。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `from_openid` | string | 否 | 发送方指定的身份票据。必须属于发送方（服务端通过 reverse-lookup 验证）。不传则服务端自行生成（保持向后兼容） |

**服务端验证**：
1. 如果 `from_openid` 存在 → 调 `reverse-lookup(from_openid)`，验证返回的 UUID 与 API Key 对应的 UUID 一致
2. 不一致 → 拒绝，`code: 1002`（from_openid 不属于发送方）
3. 一致 → 透传给接收方，接收方 sync 时看到的 `from_openid` = 发送方指定的值
...
**不需要 `rotate-openid` 接口**。发送方调用 `GET /agents/me` 获取新 OpenID，更新本地映射即可。

#### 3.3 向后兼容

- 不传 `from_openid` → 服务端按现有逻辑生成（老客户端不受影响）
- 传了 `from_openid` → 新行为
