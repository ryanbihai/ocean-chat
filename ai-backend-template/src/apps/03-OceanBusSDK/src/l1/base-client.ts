...
export class L1Client {
  ...
  /**
   * Send an L1 request to an arbitrary Agent (not just the service this client is bound to).
   * Used for Agent-to-Agent direct requests (e.g., getAgentCard).
   */
  async sendActionTo(destinationOpenid: string, request: L1Request): Promise<L1Response> {
    if (!this.dispatcher) {
      throw new OceanBusError('L1 dispatcher not configured');
    }
